...
> ```
>
> **Model**  `model/gesture_model.ptmodel` — decoded on first `predict()` call, no internet required. Model 架構（`build_model`）會從 `.ptmodel` 內記錄的 `model_module` 自動還原，`inference.py` 不需 hardcode model import。
...
├── train.py
├── model/
│   └── gesture_model.ptmodel        ← submission artifact（固定檔名）
├── checkpoints/
│   ├── <name>.pth                   ← 以 --name 命名（不再寫死 gesture_model）
│   ├── <name>_pruned50.pth
│   └── <name>_pruned50_quant.pth
├── src/
│   ├── predictor.py
│   ├── dataset.py
│   ├── evaluate.py
...
│   ├── augmentation/
│   ├── models/
│   │   ├── test.py                  ← placeholder dual-branch model
│   │   └── gcn_xattn.py             ← GCN + cross-attention model
│   └── compression/
│       ├── compress.py
│       └── baseline.py
├── config/
...
放 model architecture 定義。每個 model 檔案都要 export `build_model(model_cfg) -> nn.Module`。

- `src.models.test`：測 compression pipeline 的 placeholder dual-branch model。
- `src.models.gcn_xattn`：MobileNetV3-Small image branch + hand-skeleton GCN landmark branch + cross-attention fusion（見 Model Design）。

要用哪個 model 由 `--model_module` 指定（例如 `--model_module src.models.gcn_xattn`）。這個值會在 train 時存進 `.pth`、compress 時存進 `.ptmodel`，evaluate / inference 載入時自動 `importlib` 還原對應的 `build_model`，不需在任何地方 hardcode model import。

```txt
checkpoints/
```

放 training / compression 中間 checkpoint，不會放進最終 submission zip。檔名以 `--name` 為 stem（例如 `gcn_xattn_v1.pth`），方便區分不同實驗。
...
### src.models.gcn_xattn（目前主力 model）

在 baseline dual-branch 之上做了三個改進（皆有 2024-2025 static gesture 文獻支撐）：

```txt
crop image
→ MobileNetV3-Small features
→ 1×1 conv 投影成 spatial tokens（給 cross-attention 當 K/V）
            \
             cross-attention fusion → head → logits
            /
landmarks (21,2)
→ wrist-relative + scale normalization
→ hand-skeleton GCN（2 層 graph conv，跑在 MediaPipe 21 點拓撲上）
→ landmark embedding（當 cross-attention 的 query）
```

重點：

```txt
- Landmark branch 從 flat MLP 換成 GCN，捕捉手指關節連結關係。
  skeleton adjacency 以 Â = D^-1/2 (A+I) D^-1/2 normalize，存成固定 buffer（非參數）。
- Fusion 從 concat 換成 cross-attention：landmark embedding 當 query 去 attend
  image spatial tokens，讓手部幾何引導圖上哪些區域重要。
- 所有可學習權重都在 Conv2d / Linear，完全相容 Deep Compression（pruning + k-means
  weight sharing 只處理 Conv2d/Linear）。LayerNorm affine 以 raw tensor 全精度存進 .ptmodel。
```

參數量約 1.09M（壓縮前 ~4.36 MB fp32），壓縮後的 `.ptmodel` 遠低於 10 MB。

`build_model(model_cfg)` 接受的 keys（含 alias / 預設值）：

```txt
crop_size (112) / img_dim (128, alias image_dim) / lm_dim (64, alias landmark_dim)
gcn_hidden (64) / attn_dim (128) / head_hidden_dim (128) / dropout (0.2) / pretrained (False)
```
...
`run.py` 一個指令跑完 train → compress → evaluate，全程同一個 Python process（直接呼叫各模組 function，不用 subprocess）。所有產出以 `--name` 命名。

```powershell
python run.py --name gcn_xattn_v1 --model_module src.models.gcn_xattn `
    --epochs 30 --batch_size 512 --num_workers 12 `
    --compress_batch_size 512 --compress_num_workers 12 `
    --aug_cfg config/augmentation/default.yaml --split test
```

產出（`<P>` = `round(amount*100)`，例如 0.5 → 50）：

```txt
checkpoints/<name>.pth
checkpoints/<name>_pruned<P>.pth
checkpoints/<name>_pruned<P>_quant.pth
model/<name>.ptmodel
```
...
```txt
--skip_train      用現有 checkpoints/<name>.pth，直接 compress
--skip_compress   用現有 model/<name>.ptmodel，直接 evaluate
--skip_eval       compress 完就停
```

`--model_module` 從 train 一路帶到 compress 並寫進 `.ptmodel`，所以 evaluate / inference 會自動還原架構。

提交時 `inference.py` 固定讀 `model/gesture_model.ptmodel`（TA harness 規定的檔名）。選好實驗後：

```powershell
copy model\<name>.ptmodel model\gesture_model.ptmodel
```

架構從 `.ptmodel` 自動還原，不需改 `inference.py`。`run.py` 結尾也會印出這行提示。

> 注意：`--split` 預設 `test`。test split 的 `.npz` cache 會在 `train.py` 內自動建立（見下節）；若只跑 `--skip_train --skip_compress`，要先確保 test cache 已存在。
...
輸出（檔名預設 `gesture_model.pth`；透過 `run.py` 跑時會 rename 成 `<name>.pth`）：
...
```python
{
    ...
    "model_state_dict": ...,
    "model_cfg": ...,
    "model_module": ...,   # e.g. "src.models.gcn_xattn"，載入時自動還原 build_model
    "label_map": ...,
    "val_acc": ...,
    "aug_cfg": ...
}
...
`train.py` 用 `--model_module`（預設 `src.models.model`）選 model，並把它存進 checkpoint。`train.py` 在訓練開始前也會自動建立 `test` split 的 `.npz` cache（只建 `.npz`，不 pack `.npy`，因為 `evaluate.py` 直接讀 `.npz`），所以 evaluate 時不必再手動補 test cache。
...
指定 model（例如 GCN + cross-attention）：

```powershell
python train.py --model_module src.models.gcn_xattn --epochs 30 --batch_size 64
```

Mini subset 快速迭代（每 class 2000 張，8:1:1 切，所有 cache 放在 `data/mini_train/`）：

```powershell
python train.py --model_module src.models.gcn_xattn --mini_train --epochs 10 --batch_size 64 --num_workers 4
```

自訂 data root：

```powershell
python train.py --model_module src.models.gcn_xattn --data_root D:/datasets/hagrid --epochs 30
```

開啟 augmentation：

```powershell
python train.py --model_module src.models.gcn_xattn --aug_cfg config/augmentation/default.yaml --epochs 30
```
...
Default outputs（直接跑 `baseline.py` 時的預設；透過 `run.py` 跑時會改用 `<name>` 命名）：
...
建議透過 `run.py` 執行 compression，避免輸出和其他實驗混在一起。
...
```python
{
    ...
    "compression": {
        ...
        "model_module": "src.models.gcn_xattn",
        "prune_amount": 0.5,
        "conv_bits": 8,
        "fc_bits": 5,
        "global_sparsity": 0.5,
...
```powershell
python -m src.compression.baseline --pth_in checkpoints/gcn_xattn_v1.pth --model_module src.models.gcn_xattn --mini_train --prune_epochs 1 --ft_epochs 1
```

正式資料（全量）：

```powershell
python -m src.compression.baseline --pth_in checkpoints/gcn_xattn_v1.pth --model_module src.models.gcn_xattn
```

`--model_module`（預設 `src.models.test`）指定重建 model 用的架構，並寫進輸出的 `.ptmodel` meta。輸出路徑可用 `--pruned_pth_out` / `--quant_pth_out` / `--ptmodel_out` 自訂；透過 `run.py` 跑時會自動以 `<name>` 命名。

自訂 data root：

```powershell
python -m src.compression.baseline --pth_in checkpoints/gcn_xattn_v1.pth --model_module src.models.gcn_xattn --data_root D:/datasets/hagrid
```
...
三種都透過 `GesturePredictor` 推論。`.pth` 和 `.ptmodel` 載入時，若沒有顯式傳入 `model_builder`，會從檔案內記錄的 `model_module` 自動 `importlib` 還原 `build_model`；因此可以直接比較壓縮前（`.pth`）與壓縮後（`.ptmodel`）的表現。

> 舊版 train.py（沒有寫入 `model_module`）存的 `.pth` 不含這個欄位，載入會報錯。可手動補：
> ```python
> import torch
> c = torch.load("checkpoints/<name>.pth", map_location="cpu")
> c["model_module"] = "src.models.gcn_xattn"
> torch.save(c, "checkpoints/<name>.pth")
> ```

跟其他 entry 一樣只收 `--data_root`（default `data`），cache 讀 `<data_root>/processed`；
加 `--mini_train` 則讀 `<data_root>/mini_train/processed`。

Evaluate original checkpoint（壓縮前）：

```powershell
python -m src.evaluate --weights checkpoints/gcn_xattn_v1.pth --split test
```

Evaluate compressed `.ptmodel`（壓縮後）：

```powershell
python -m src.evaluate --weights model/gcn_xattn_v1.ptmodel --split test
```
...
```powershell
python -m src.evaluate --weights model/gcn_xattn_v1.ptmodel --mini_train --split test
```
