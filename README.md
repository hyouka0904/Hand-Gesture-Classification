...
```powershell
python -m src.train --ann_root data/annotations --image_root data/hagridv2_512 --cache_root data/processed --epochs 30 --batch_size 64 --num_workers 0 --aug_cfg config/augmentation/default.yaml --output_dir checkpoints
```
...
例如 `batch_size=16` 代表每次 forward / backward 使用 16 筆樣本更新一次參數。
